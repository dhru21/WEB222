...
function createEmailAddress(firstName, lastName, host) {
  ...
  let email;
  if (host) {
    nameStr = firstName.charAt(0).toLowerCase() + crush(lastName);
    email = `${firstName} ${lastName} <${nameStr}@${host}>`;
  } else {
    nameStr = firstName.charAt(0).toLowerCase() + crush(lastName);
    email = `${firstName} ${lastName} <${nameStr}@gmail.com>`;
  }
  return email;
}
...
function ageToRange(age) {
  var range;
  if (age >= 0 && age <= 19) {
    range = `19 and younger`;
  } else if (age >= 20 && age <= 29) {
    range = `20 to 29 Years`;
  } else if (age >= 30 && age <= 39) {
    range = `30 to 39 Years`;
  } else if (age >= 40 && age <= 49) {
    range = `40 to 49 Years`;
  } else if (age >= 50 && age <= 59) {
    range = `50 to 59 Years`;
  } else if (age >= 60 && age <= 69) {
    range = `60 to 69 Years`;
  } else if (age >= 70 && age <= 79) {
    range = `70 to 79 Years`;
  } else if (age >= 80 && age <= 89) {
    range = `80 to 89 Years`;
  } else if (age >= 90 && age <= 125) {
    range = `90 and older`;
  } else {
    throw `invalid age`;
  }
 ...
 ******************************************************************************/
function simpleMovingAverage(...Number) {
  let madeInt;
  let sumInt = 0;
  let totalInt = 0;
  let sum = 0;
  for (let i = 0; i < Number.length; i++) {
    if (typeof Number[i] === 'string' || typeof Number[i] === 'boolean') {
      madeInt = parseInt(Number[i]);
      totalInt += madeInt;
    } else {
      sumInt += Number[i];
    }
...
function mimeFromExt(ext) {
  switch (ext.startsWith('.') ? ext.slice(1) : ext) {
    case 'txt':
      return 'text/plain';
    case 'html':
      return 'text/html';
...
function normalizeBoolean(value) {
  ...
  if (arrTrue.includes(value)) {
    value = true;
    return value;
  }

  if (!arrElse.includes(value)) {
...
function formatQS(id, neighbourhood) {
  return neighbourhood
    ? `?id=${encodeURIComponent(id)}&n=${encodeURIComponent(neighbourhood)}`
...
function containsID(id, qs) {
  let deQs = decodeURIComponent(qs);
  let number;
  let start = deQs.indexOf('=');
  let end;
  deQs.includes('&') ? (end = deQs.indexOf('&')) : (end = null);
  end
    ? (number = parseInt(deQs.substr(start + 1, end - (start + 1))))
    : (number = parseInt(deQs.substr(start + 1)));
